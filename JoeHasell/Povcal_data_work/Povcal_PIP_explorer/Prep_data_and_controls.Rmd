...
-----
Compare original and estimated Ginis

```{r}

df_plot<- df %>%
  select(Entity, Year, Gini.index, Gini_estimated) %>%
  rename(Gini_original = Gini.index) %>%
  pivot_longer(cols = c( Gini_original, Gini_estimated), names_to = "measure", values_to = "value") %>%
  filter(Entity == "NAM")


ggplot(data = df_plot, aes(x=Year, y=value, colour = measure) ) +
  geom_line() +
  ggtitle("Original and estimated Gini Coefficient for Botswana from PIP")
