...
Read in explorer control stubs
```{r}
control_stubs <- read.csv("https://raw.githubusercontent.com/owid/notebooks/main/JoeHasell/Copy_of_Michalis_work/control_stubs.csv")

# For some reason it's grabbing lots of empty rows/cols as variables -  drop these

control_stubs<- control_stubs[, colSums(is.na(control_stubs)) != nrow(control_stubs)]

control_stubs<- control_stubs[rowSums(control_stubs=="") != ncol(control_stubs),]
...
# This function replaces the key words across a dataframe for a line of a given aux sheet
myReplaceFun<- function(df_block, replacements_row){
  
  replacements_row<- list_aux_sheet[[1]]
  
  search_for_strs<- paste0("[", aux_sheet, "$", names(replacements_row), "]")

  df_block_replaced<- df_block
    for(i in 1:length(search_for_strs)){
      
      
      search_for_str<- search_for_strs[[i]]
      replace_with_str<- replacements_row[1,i]
        
      df_block_replaced<- data.frame(lapply(df_block_replaced, function(x) {gsub(search_for_str, replace_with_str, x)}))
