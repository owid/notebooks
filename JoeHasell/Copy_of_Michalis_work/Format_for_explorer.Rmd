...
explorer_vars[["poverty"]]<- paste0(pov_stub_line_combos$poverty_var_stubs, "_", pov_stub_line_combos$pov_lines)
...
For each explorer, filter the data for the specified vars, flip back to wide format, and then split by welfare/datatype and save as separate csvs
```{r}

for(explorer in explorer_vars)

check<- df %>% filter(new_varnames %in% explorer_vars[["poverty"]]) 
