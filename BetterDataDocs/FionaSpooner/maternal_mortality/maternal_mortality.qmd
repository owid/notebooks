...
```
Find countries where there are values from more than one source i.e. there are duplicate country-year rows. Where there are duplicate values sourced from WHO we drop these. 
...
```{python}
df = df[~((df['entity'] == 'Sri Lanka') & (df['source'] == 'gapminder'))]
```
