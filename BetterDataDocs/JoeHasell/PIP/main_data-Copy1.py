    ...
    for is_filled in ['true', 'false']:
        ...
        for ent_type in ['country', 'region']:
            ...
            if ent_type == 'country':
                ...
                df = df.rename(columns={'country_name': 'EntityName'})
            ...
            if ent_type == 'region':
                ...
                df = df.rename(columns={'region_name': 'EntityName'})
...
for is_filled in ['true', 'false']:
    ...
    headcounts_country_wide = headcounts_country.pivot_table(index=['EntityName', 'reporting_year','reporting_level','welfare_type'], 
                    columns='poverty line')

    headcounts_region_wide = headcounts_region.pivot_table(index=['EntityName', 'reporting_year'], 
                    columns='poverty line')
    ...
    df_final = standardize_entities(
        ...
        data_varname_old = 'EntityName',
        data_varname_new = 'EntityName'
    )

    # Amend the EntityName to reflect if data refers to urban or rural only
    df_final.loc[(\
        df_final['reporting_level'].isin(["urban", "rural"])),'EntityName'] = \
        df_final.loc[(\
        df_final['reporting_level'].isin(["urban", "rural"])),'EntityName'] + \
            ' - ' + \
        df_final.loc[(\
    ...
    col_ids = ['EntityName', 'year', 'reporting_level', 'welfare_type', 'reporting_pop']
    col_avg_shortfall = []
    col_headcount = []
    col_headcount_ratio = []
    col_incomegap = []
    ...
    df_inc_or_cons.sort_values(by=['EntityName', 'year', 'reporting_level', 'welfare_type'], ignore_index=True)
    df_inc_or_cons['duplicate_flag'] = df_inc_or_cons.duplicated(subset=['EntityName', 'year', 'reporting_level'])
