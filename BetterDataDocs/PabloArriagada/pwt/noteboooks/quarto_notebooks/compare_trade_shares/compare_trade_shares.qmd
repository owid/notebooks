...
We would like to produce a series for trade openness – measured as imports plus exports as a share of GDP. Our main interest for calculating this is to then produce a global aggregate measure of trade openness – for use in our [Globalization over 5 centuries](https://ourworldindata.org/grapher/globalization-over-5-centuries-km) chart.

At first glance, this seems possible to calculate from either of the two data files in _PWT_. However they yield different results.
...
And we explain why we have decided to use the National Accounts data in our charts of trade openness.
...
#### Trade openness at the country level
...
::: {.callout-note appearance="simple"}
## What is the denominator of the trade share variables in the main PWT data file?
...
df_exports_na = df_na[['year', 'entity']]
df_imports_na = df_na[['year', 'entity']]

#Current price shares
df_exports_na['National Accounts – current price'] = df_na['v_x']/df_na['v_gdp'] 
df_imports_na['National Accounts – current price'] = df_na['v_m']/df_na['v_gdp'] 

#Prep trade shares from main data
df_exports_main = df_main[['year', 'entity']]
df_imports_main = df_main[['year', 'entity']]

df_exports_main['Main data']= df_main['csh_x']
df_imports_main['Main data']= df_main['csh_m']


# Reshape for plot
df_imports = pd.concat([df_imports_na,df_imports_main])
df_imports['measure'] = 'imports'

df_exports = pd.concat([df_exports_na,df_exports_main])
df_exports['measure'] = 'exports'

df_plot = pd.concat([df_imports,df_exports])


# Facet scatter to compar

fig = px.scatter(df_plot, x = 'National Accounts – current price', y="Main data", color = 'entity', facet_col = 'measure')
fig.show()
...
#Current price shares
df_imports['Current prices'] = df_na['v_m']/df_na['v_gdp'] 
df_exports['Current prices'] = df_na['v_x']/df_na['v_gdp'] 

#Constant price shares
df_imports['Constant prices'] = df_na['q_m']/df_na['q_gdp'] 
df_exports['Constant prices'] = df_na['q_x']/df_na['q_gdp'] 
