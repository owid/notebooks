...
df_na['import_share_current_prices'] = df_na['v_x']/df_na['v_gdp'] 
df_na['export_share_current_prices'] = df_na['v_m']/df_na['v_gdp'] 

#Constant price shares
df_na['import_share_constant_prices'] = df_na['q_x']/df_na['q_gdp'] 
df_na['export_share_constant_prices'] = df_na['q_m']/df_na['q_gdp'] 

# Reshape for plot
df_plot = df_na[['year','entity','import_share_current_prices', 'export_share_current_prices', 'import_share_constant_prices', 'export_share_constant_prices']]
...
We see that the shares calculated according to the current and constant price data are similar, but not identical. Here we plot the ratio of the current to constant price shares for both imports and exports. Whilst the ratio is generally close to one, there is a large range.


```{python}
#| code-fold: true
#| code-summary: "Compare constant and current prices plot"

#Calculate ratio of current to constant price shares
df_na['import_ratio'] = df_na['import_share_current_prices']/df_na['import_share_constant_prices'] 
df_na['export_ratio'] = df_na['export_share_current_prices']/df_na['export_share_constant_prices'] 

# Reshape for plot
df_plot = df_na[['year','entity','import_ratio', 'export_ratio']]

df_plot = df_plot.melt(id_vars=['year','entity'], var_name='measure')

fig = px.box(df_plot, x = 'measure', y="value", range_y=[-0.1,10])
fig.show()

```


Again we can calculate a global aggregate. But we have to convert currencies....
