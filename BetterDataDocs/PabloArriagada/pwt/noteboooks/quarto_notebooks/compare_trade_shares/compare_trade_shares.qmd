...
In computing the global aggregate for the current price data, we drop one of the two China series so as to not overly weight China in the calculation.
...
### Global aggregate trade openness with the national accounts data

As the national accounts data are given in local currencies, to calculate a global aggregate we need to use a common currency. The national accounts file provides exchange rates (national currency/USD) for this purpose.
...
# Weighted average with incomplete coverage
world_trade_openness_na = df_na.dropna(subset=['trade_openness', 'v_gdp_usd'], how = 'all').groupby("year").apply(lambda x: np.average(x['trade_openness'], weights=x['v_gdp_usd'])).reset_index()
...
world_trade_openness_main_alt['source'] = "Main data – international-$ (single benchmark)"

# Stack together
df_plot= pd.concat([world_trade_openness_na,world_trade_openness_main_alt])
