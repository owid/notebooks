...
- A [read-only version](https://htmlpreview.github.io/?https://raw.githubusercontent.com/owid/notebooks/main/BetterDataDocs/PabloArriagada/pwt/noteboooks/analysis_notebooks/compare_trade_shares/compare_trade_shares.html) online
- A script in [GitHub](https://github.com/owid/notebooks/blob/main/BetterDataDocs/PabloArriagada/pwt/noteboooks/analysis_notebooks/compare_trade_shares/compare_trade_shares.qmd)
...
We also drop an alternative set of series included in the _PWT_ National Accounts data for China. (The alternative series only relates to the inflation-adjusted data, which we are not using here – so in dropping the alternative series we are simply removing duplicate observations.)
...
The plot below shows which countries have the data available to be included in the world aggrgeation in each year.

**JH comment**: Pablo could you please add the coverage chart so we can see the coverage of which countries are included in the NA aggregation above in any given year.
