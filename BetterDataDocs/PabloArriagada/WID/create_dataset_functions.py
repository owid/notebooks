...
def load_and_standardize(file):
    ...
    df_final = standardize_entities(
        df_final,
        CURRENT_DIR / "data/raw/countries_country_standardized.csv",
        "country",
...
def add_metadata_and_export(df_final, sheet):
    print(
        f"Changing the names of variables and exporting Grapher's dataset ({sheet})..."
    )
    start_time = time.time()
    ...
    df_dataset = df_dataset.set_index(
        ["Entity", "Year"], verify_integrity=True
    ).sort_index()

    # Export the dataset
    df_dataset.to_csv(f"{CURRENT_DIR}/data/final/{sheet}.csv")
...
def create_faceted_dataset(df_final):
    print(
        f"Creating a dataset to compare the evolution of the top 1% share in two groups..."
    )
    start_time = time.time()
    ...
    df_english = (
        df_final[df_final["Entity"].isin(english_countries)]
        .reset_index(drop=True)
        .copy()
    )
    df_europe_jp = (
        df_final[df_final["Entity"].isin(europe_japan)].reset_index(drop=True).copy()
    )

    df_english = df_english.rename(
        columns={"p99p100_share_pretax": "Top 1% share in English speaking countries"}
    )
    df_europe_jp = df_europe_jp.rename(
        columns={"p99p100_share_pretax": "Top 1% share in Continental Europe and Japan"}
    )
...
def create_faceted_dataset_temp():
    ...
    df = df[
        (df["measure"] == "Share of top 1%")
        & (
            (df["welfare_concept"] == "Pre-tax national income (equal-split adults)")
            | (df["welfare_concept"] == "Pre-tax national income (equal-split adult)")
        )
    ]
    ...
    df = df.rename(columns={"year": "Year", "value": "p99p100_share_pretax"})
